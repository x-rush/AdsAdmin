...
public class MatchEnrollActivity extends AppCompatActivity {
    @BindView(R.id.image)
    ImageView image;
    ...
    private void initData() {
        Glide.with(MyApplication.getAppContext())
                .load(MyApplication.getImagePath() + getIntent().getStringExtra("match_pic"))
                .into(image);
    ...
    private void enroll11() {
        ...
        Call<StatusBean> call = service.enroll(getSharedPreferences("yj", MODE_PRIVATE).getString("id", "0"),
                getIntent().getStringExtra("match_id"), tv_num.getText().toString());
